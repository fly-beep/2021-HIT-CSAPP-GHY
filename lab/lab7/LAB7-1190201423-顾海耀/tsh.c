...
int builtin_cmd(char **argv) 
{
    ...
    if(!strcmp(argv[0],"bg") || !strcmp(argv[0],"fg"))  //调用do_bgfg
    {
	do_bgfg(argv);
	return 1;
    }
...
void waitfg(pid_t pid)
{
    while(pid == fgpid(jobs))	//若当前进程PID与当前前台进程的PID相等，则进行挂起
    {
	sleep(1);	//休眠挂起
    }
    return;
...
void sigchld_handler(int sig) 
{
    ...
    int stat;
    pid_t pid;
    sigfillset(&sigg);	//将所有信号添加到sigg中
    while((pid = waitpid(-1,&stat,WNOHANG | WUNTRACED)) > 0)	//回收子进程
    {
	sigprocmask(SIG_BLOCK,&sigg,&pr_sigg);	//阻塞所有信号
	jobb = getjobpid(jobs,pid);	//通过pid找到操作job
	if (WIFSTOPPED(stat)) //如果子进程停止则返回
	{
	    jobb->state = ST;
	    printf("Job [%d] (%d) terminated by signal %d\n", jobb->jid, jobb->pid, WSTOPSIG(stat));
	}
	else
	{
	    if (WIFSIGNALED(stat))  //子进程终止
	    {
	        printf("Job [%d] (%d) terminated by signal %d\n", jobb->jid, jobb->pid, WTERMSIG(stat));
	    }
